...
// /sbv1p1 — Copper/Leather redesign. Scope: landing page only. See
// docs/sbv1p1-copy-deck.md for the copy source of truth and
...
function trackPixel(
  ...
  fetch("/api/pixel-event", {
    ...
    body: JSON.stringify({
      ...
      landing_page:
        landingPage || (typeof window !== "undefined" ? window.location.pathname : "/sbv1p1"),
      source: "client",
    }),
  }).catch(() => {});
}
...
function scoreTone(score: number): { fill: string; label: string } {
  ...
  if (score >= 70) return { fill: "#f472b6", label: "fixable" };
...
export default function Page() {
  ...
  const [sport, setSport] = useState<"baseball" | "softball">("softball");
  ...
  // 10-min window stored in localStorage. While active, variant prices
  // ($9.99/$19.99/$14.99 Pro) apply with strike-throughs. When expired,
  // prices swap to home-tier ($14.99/$29.99/$19.99 Pro) and buyPlan/
  // handleSubscribe send tier:"home" so Stripe charges the higher prices.
  // Resets after 24h so a returning visitor the next day gets a fresh
  // window — keeps the urgency honest without permanently locking out
  // anyone who comes back tomorrow.
  const URGENCY_DURATION_S = 600;
  const URGENCY_RESET_MS = 24 * 60 * 60 * 1000;
  const [urgencySeconds, setUrgencySeconds] = useState(URGENCY_DURATION_S);
  const [urgencyExpired, setUrgencyExpired] = useState(false);

  useEffect(() => {
    const KEY = "armiq_urgency_start";
    const now = Date.now();
    let startMs: number;
    try {
      const stored = window.localStorage.getItem(KEY);
      const parsed = stored ? parseInt(stored, 10) : NaN;
      if (Number.isFinite(parsed) && now - parsed <= URGENCY_RESET_MS) {
        startMs = parsed;
      } else {
        startMs = now;
    ...
    function tick() {
      ...
      const remaining = URGENCY_DURATION_S - elapsed;
      if (remaining <= 0) {
        setUrgencySeconds(0);
        setUrgencyExpired(true);
      } else {
        setUrgencySeconds(remaining);
        setUrgencyExpired(false);
      }
    }
    tick();
    ...
    if (params.get("demo") === "true") {
      setResult({
        ...
        top3: [
          ...
          "Front foot lands open — gets beat by the rise ball",
        ],
        impact_line: "Mechanical inefficiency limiting velocity and consistency.",
        uplift_line: "Fixing these could add 3–6 mph velocity and turn pop-ups into line drives.",
      });
      setPitchId("demo");
      setAnalysisSaved(true);
      window.history.replaceState({}, "", "/sbv1p1");
    }
    ...
    try {
      const res = await fetch("/api/checkout", {
        ...
        body: JSON.stringify({
          ...
          utm: utmData,
          landing_page: landingPage,
          // tier:"home" tells the API to charge the higher home prices
          // when the discount countdown has already expired on this page.
          tier: urgencyExpired ? "home" : undefined,
        }),
      });
  ...
  async function handleSubscribe() {
    ...
    try {
      ...
      trackPixel(
        "InitiateCheckout",
        { content_name: "ArmIQ Pro Monthly", currency: "USD", value: urgencyExpired ? 19.99 : 14.99 },
        email,
        landingPage
      );
      window.location.href = url;
  ...
  // Pull the user's #1 leak out of result.top3[0] for personalized
  // pricing copy + day-1 teaser. The leak text is shaped
  // "<leak> — <consequence>" so the substring before the em-dash is
  // the leak label. Falls back to a safe default for missing/malformed data.
  const userTop1 = result?.top3?.[0]?.split("—")[0]?.trim() || "your top leak";
  const offersEnabled = analysisSaved && !!pitchId;

  const urgencyMM = String(Math.floor(urgencySeconds / 60)).padStart(2, "0");
  const urgencySS = String(urgencySeconds % 60).padStart(2, "0");
      ...
      <section className={s.hero}>
        ...
        <div className={s.heroInner}>
          <div className={s.heroLeft}>
            <div className={s.kicker}>AI pitch analysis · Built for softball</div>
            ...
            <p className={s.heroSub}>
              ...
              mechanical leaks, and a drill plan built from her frames. Most pitchers
          ...
          <aside className={s.heroSample} aria-label="Example pitch score">
            ...
            <div className={s.sampleFixes}>
              ...
              <ol className={s.sampleFixList}>
                ...
                <li>Front foot lands open on the rise ball</li>
              </ol>
            </div>
          </aside>
        </div>
      ...
      {loading ? (
        ...
        <section ref={formRef} className={s.formSection}>
          <div className={s.formInner}>
            ...
            <h2 className={s.sectionHead}>One pitch. Three fixes. A plan built for her.</h2>
        ...
        <>
          ...
          <section ref={offersRef} className={s.plansSection}>
            <div className={s.plansInner}>
              <div className={s.planUrgency}>
                <span className={s.planUrgencyDot} aria-hidden>●</span>
                {urgencyExpired ? (
                  <>
                    <strong>Discounts expired</strong> — prices have reset to full
                  </>
                ) : (
                  <>
                    <strong>Discounts expire in{" "}
                    <span className={s.planUrgencyTimer}>{urgencyMM}:{urgencySS}</span></strong>{" "}
                    — this page only
                  </>
                )}
              </div>

              <div className={s.planTeaser}>
                <div className={s.planTeaserKicker}>What Day 1 looks like</div>
                <div className={s.planTeaserHead}>
                  Targets your <em>{userTop1.toLowerCase()}</em>
                </div>
                <ol className={s.planTeaserList}>
                  <li>Warm-up: hip mobility + bat-path activation (3 min)</li>
                  <li>Drill 1: targeted fix for your #1 leak (4 sets × 8)</li>
                  <li>Drill 2: sequencing — load → swing pattern (3 sets × 6)</li>
                  <li>Drill 3: live tee with a coaching cue tied to your pitch</li>
                </ol>
                <div className={s.planTeaserNote}>
                  ↓ Unlock the full plan below — built from your frames
                </div>
              </div>
              ...
              <div className={s.planGrid}>
                <PlanCard
                  ...
                  price={urgencyExpired ? 14.99 : 9.99}
                  strikePrice={urgencyExpired ? undefined : 14.99}
                  sub="The fastest way to feel a difference"
                  features={[
                    `Day 1 targets: ${userTop1.toLowerCase()}`,
                    "3 drills/day + warm-up, coaching cues, rep counts",
                    "Player dashboard for 7 days",
                  ]}
                  ctaText="Start 7-day fix"
                  subCta="If you don't feel a difference in 7 days, full refund — no questions"
                  enabled={offersEnabled}
                  onClick={() => buyPlan(7, urgencyExpired ? 14.99 : 9.99, "7-Day Quick Fix")}
                />
                <PlanCard
                  tier="14-Day Fix"
                  badge="Most chosen"
                  tagline="Fix all three leaks"
                  price={urgencyExpired ? 29.99 : 19.99}
                  strikePrice={urgencyExpired ? undefined : 29.99}
                  sub="Build the fix, then lock it in"
                  features={[
                    `Starts with your #1 leak: ${userTop1.toLowerCase()}`,
                    "Week 1 isolation → Week 2 sequencing on all 3 leaks",
                    "3 drills/day with warm-up, cues, progression reps",
                    "Player dashboard + score tracking",
                  ]}
                  ctaText="Start 14-day fix"
                  subCta="If your score doesn't move 5+ points by day 14, full refund — and we'll tell you why"
                  enabled={offersEnabled}
                  featured
                  onClick={() => buyPlan(14, urgencyExpired ? 29.99 : 19.99, "14-Day Fix")}
                />
              </div>
              ...
              <div className={s.proCard}>
                <div className={s.proLeft}>
                  ...
                  <div className={s.proPrice}>
                    {!urgencyExpired && <span className={s.proPriceStrike}>$19.99</span>}
                    <span className={s.proPriceNum}>{urgencyExpired ? "$19.99" : "$14.99"}</span>
              ...
              <div className={s.proofGrid}>
                <figure className={s.quoteCard}>
                  <blockquote className={s.quote}>
                    "Thought it was gimmicky. The three fixes were the same things her
                    hitting coach had been saying for months — she didn't buy in
                    until she saw the score. Bat speed up, two more line drives
                    per game."
                  </blockquote>
                  <figcaption className={s.quoteAttr}>
                    <span>Jess M. · mom of a 14U fastpitch pitcher</span>
                    <span className={s.quoteRole}>Phoenix, AZ</span>
                  </figcaption>
                </figure>

                <figure className={s.quoteCard}>
                  <blockquote className={s.quote}>
                    "I don't hand my girls anything an AI made. This one's the
                    exception. Barrel-path read is accurate and the drill
                    progression isn't dumb — matches what I'd program. Using
                    it between lessons."
                  </blockquote>
                  <figcaption className={s.quoteAttr}>
                    <span>Coach Walker · 14U fastpitch travel</span>
                    <span className={s.quoteRole}>Dallas area</span>
                  </figcaption>
                </figure>
